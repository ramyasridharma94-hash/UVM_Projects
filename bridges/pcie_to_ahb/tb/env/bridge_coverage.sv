`ifndef AHB_BRIDGE_COVERAGE_SV
`define AHB_BRIDGE_COVERAGE_SV
class bridge_coverage extends uvm_subscriber #(pcie_tlp_seq_item);
  ...
  covergroup cg_length;
    cp: coverpoint item.length { bins one={1}; bins small={[2:4]}; bins med={[5:8]}; }
  endgroup
  covergroup cg_addr_space;
    cp: coverpoint item.addr[63:32] { bins lo32={32'h0}; bins hi64=default; }
  endgroup
  covergroup cg_burst_len;
    cp: coverpoint item.length { bins single={1}; bins incr4={[2:4]}; bins incr8={[5:8]}; bins incr={[9:$]}; }
  endgroup

  function new(string name, uvm_component parent);
    super.new(name, parent);
    cg_tlp_type=new(); cg_length=new(); cg_addr_space=new(); cg_burst_len=new();
  endfunction
  function void write(pcie_tlp_seq_item t);
    item=t; cg_tlp_type.sample(); cg_length.sample(); cg_addr_space.sample(); cg_burst_len.sample();
  endfunction
  function void report_phase(uvm_phase phase);
    `uvm_info("COV",$sformatf(
      "\n=== AHB Bridge Coverage ===\n  TLP Type:%.1f%%  Length:%.1f%%  AddrSpace:%.1f%%  Burst:%.1f%%",
      cg_tlp_type.get_inst_coverage(),cg_length.get_inst_coverage(),
      cg_addr_space.get_inst_coverage(),cg_burst_len.get_inst_coverage()),UVM_LOW)
  endfunction
endclass
`endif
