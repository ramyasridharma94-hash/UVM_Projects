`ifndef AHB_PCIE_TLP_MONITOR_SV
`define AHB_PCIE_TLP_MONITOR_SV
    ...
    fork
      forever begin @(vif.monitor_cb);
        if (vif.monitor_cb.req_valid && vif.monitor_cb.req_ready) begin
          ...
          it.first_be=vif.monitor_cb.req_first_be; it.data=vif.monitor_cb.req_data;
          ap_req.write(it);
        end
      end
      forever begin @(vif.monitor_cb);
        if (vif.monitor_cb.cpl_valid) begin
          ...
          it.tag=vif.monitor_cb.cpl_tag; it.data={32'h0,vif.monitor_cb.cpl_data};
          ap_cpl.write(it);
        end
      end
    join
