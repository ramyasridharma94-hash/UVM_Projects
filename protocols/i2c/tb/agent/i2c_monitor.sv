...
class i2c_monitor extends uvm_monitor;
  ...
  function void build_phase(uvm_phase phase);
    ...
    if (!uvm_config_db #(virtual i2c_if.monitor_mp)::get(this, "", "vif", vif))
      `uvm_fatal("CFG", "I2C monitor: no vif")
  endfunction
  ...
  task run_phase(uvm_phase phase);
    ...
    forever begin
      ...
      @(vif.monitor_cb);
      prev_scl = vif.monitor_cb.scl;
      prev_sda = vif.monitor_cb.sda;
      @(vif.monitor_cb);
      if (prev_scl && vif.monitor_cb.scl && prev_sda && !vif.monitor_cb.sda) begin
        ...
        addr_byte = '0;
        repeat (8) begin
          @(posedge vif.monitor_cb.scl);
          addr_byte = {addr_byte[6:0], vif.monitor_cb.sda};
        end
        item.op         = addr_byte[0] ? I2C_READ : I2C_WRITE;
        ...
        repeat (8) begin
          @(posedge vif.monitor_cb.scl);
          item.reg_addr = {item.reg_addr[6:0], vif.monitor_cb.sda};
        end
        @(negedge vif.monitor_cb.scl); // skip ACK

        ap.write(item);
        `uvm_info("I2C_MON", $sformatf("%s", item.convert2string()), UVM_HIGH)
      end
    end
  endtask
