...
class apb_monitor extends uvm_monitor;
  ...
  function void build_phase(uvm_phase phase);
    ...
    if (!uvm_config_db #(virtual apb_if.monitor_mp)::get(this, "", "vif", vif))
      `uvm_fatal("CFG", "APB monitor: no vif")
  endfunction

  task run_phase(uvm_phase phase);
    apb_seq_item item;
    forever begin
      // Capture on ENABLE phase with PREADY
      @(vif.monitor_cb iff (vif.monitor_cb.psel    &&
                            vif.monitor_cb.penable  &&
                            vif.monitor_cb.pready));
      item         = apb_seq_item::type_id::create("item");
      item.op      = vif.monitor_cb.pwrite ? APB_WRITE : APB_READ;
      ...
      ap.write(item);
      `uvm_info("APB_MON", $sformatf("%s", item.convert2string()), UVM_HIGH)
    end
  endtask
